...
> [`RMBENCH_ROBOTTT_CURRENT.md`](RMBENCH_ROBOTTT_CURRENT.md). In particular, do
...
- Repository: `/mnt/cpfs/fx/code/projects/Isaac-GR00T`
- Python environment: `/mnt/cpfs/fx/envs/gr00t-n1.7`
- Base weights: `/mnt/cpfs/fx/models/GR00T-N1.7-3B`
- RMBench LeRobot data: `/mnt/oss/fx/RMBench/lerobot_v2.1`
- Checkpoints and short-lived caches: `/mnt/cpfs/fx/...`

The activation script explicitly unsets `HF_ENDPOINT`, so any future download
uses the official Hugging Face endpoint rather than an HF mirror:
...
```bash
bash scripts/ppu/run_robottt_inference_smoke.sh
bash scripts/ppu/run_robottt_sequence_train_smoke.sh
```

The sequence smoke loads the real 3B model, reads two consecutive frames from
the OSS sample dataset, performs two recurrent fast-weight updates, and runs a
real BF16 backward pass. It deliberately does not save another multi-gigabyte
checkpoint.
...
python gr00t/experiment/launch_finetune.py \
  --base-model-path /mnt/cpfs/fx/models/GR00T-N1.7-3B \
  --dataset-path /mnt/oss/fx/RMBench/lerobot_v2.1 \
  --embodiment-tag NEW_EMBODIMENT \
  --modality-config-path examples/RMBench/rmbench_config.py \
  --output-dir /mnt/cpfs/fx/runs/rmbench-all-ttt-l32 \
  --num-gpus 1 \
  --global-batch-size 1 \
  --use-ttt \
  --tune-ttt \
...
python gr00t/eval/run_gr00t_server.py \
  --model-path /mnt/cpfs/fx/runs/rmbench-all-ttt-l32/checkpoint-N \
  --embodiment-tag NEW_EMBODIMENT \
  --host 0.0.0.0 \
  --port 5555
```
...
screened on held-out expert demonstrations with a paired causal probe:

```bash
GR00T_PPU_DEVICE=14 \
GR00T_OFFLINE_CHECKPOINT=/path/to/checkpoint-100 \
/bin/bash scripts/ppu/run_rmbench_offline_memory_eval.sh
```
