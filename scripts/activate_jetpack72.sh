#!/bin/bash
# activate_jetpack72.sh — Source this in each new shell to configure a JetPack 7.2 environment.
# Usage: source scripts/activate_jetpack72.sh
#
# Sets TRITON_PTXAS_PATH and LD_LIBRARY_PATH needed for inference on Jetson Orin and Thor.
# Docker users don't need this — the Dockerfile sets these via ENV directives.

# torch.compile needs ptxas from the CUDA toolkit
if [ -f /usr/local/cuda-13.2/bin/ptxas ]; then
...
# Ensure CUDA 13.2 runtime is loaded for the PyTorch cu132 stack.
...
# Add CUDA, NVIDIA, and TensorRT pip package libs to LD_LIBRARY_PATH.
SITE_PKGS="$(python3 -c 'import site; print(site.getsitepackages()[0])' 2>/dev/null || true)"
if [ -n "${SITE_PKGS:-}" ]; then
    EXTRA_LIB_DIRS=$(find "$SITE_PKGS" \
        \( -path "$SITE_PKGS/nvidia/*/lib" \
            -o -path "$SITE_PKGS/cuda*/lib" \
            -o -path "$SITE_PKGS/tensorrt_libs" \
            -o -path "$SITE_PKGS/tensorrt*/lib" \) \
...
echo "JetPack 7.2 environment configured."
